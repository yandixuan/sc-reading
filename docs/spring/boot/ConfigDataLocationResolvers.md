...
```java
 List<ConfigDataResolutionResult> resolve(ConfigDataLocationResolverContext context, ConfigDataLocation location,
   ...
   * 例如StandardConfigDataLocationResolver则是SpringBoot内置的加载`application`配置文件的类(通过`propertySourceLoaders`实现对不同文件后缀的加载，由相应的`ConfigDataLoader`完成对资源的转换，读取到propertyResource列表)
   * NacosConfigDataLocationResolver则是通过实现接口`ConfigDataLocationResolver`完成对配置中心配置的location的解析读取再转交给`NacosConfigDataLoader`读取到propertyResource列表继而完成第三方import的配置的加载
   */
